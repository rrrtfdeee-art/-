...
def generate_batch_claude_prompt(novel_name: str = "After Severing Ties") -> str:
    ...
    # ── البرومبت الروائي الذهبي الموحّد مع المحددات الصارمة
    intro_lines = [
        f"أنت \"كبير محرري الروايات ومتقن الترجمة الأدبية العربية الرفيعة\".",
        f"مهمتك: صقل وإعادة صياغة الفصول التالية من رواية [{novel_name}] وتحويلها من مسودة مترجمة إلى نص روائي عربي فصيح، عذب، وممتع.",
        "",
        "══════════════════════════════════════════════════",
        "⛔ [المحددات الصارمة - أي مخالفة تُعد فشلاً للمهمة]:",
        "══════════════════════════════════════════════════",
        "",
        "1. الأسلوب الأدبي:",
        "   - أزل أي ركاكة أو أثر للترجمة الحرفية (تخلص من: 'هو قال'، 'في تلك اللحظة'، 'كان غير قادر على ألا...').",
        "   - نوّع في الإيقاع: جمل حادة سريعة للمعارك، ووصف بلاغي ثري للمشاعر والأجواء.",
        "   - الحوارات يجب أن تكون حية وطبيعية، لا مسرحية مصطنعة.",
        "",
        "2. الضمائر والمصطلحات:",
        "   - التزم بالترجمات العربية المعتمدة من القاموس أدناه دون أي تغيير أو استبدال.",
        "   - تصحيح تصريف الضمائر (مذكر/مؤنث) بدقة متناهية بناءً على جنس الشخصية [ذكر/أنثى].",
        "",
        "3. وسوم التنسيق المسموح بها حصراً (أكواد تصميم الموقع فقط):",
        "   [system]...[/system] ← للإشعارات والواجهات النظامية",
        "   [system red]...[/system] ← للتحذيرات والأخطار",
        "   [rift]...[/rift] ← للأصوات الغامضة من بُعد آخر",
        "   [doc title=\"...\"]...[/doc] ← للكتب والوثائق",
        "   [tip]...[/tip] أو [note]...[/note] ← للملاحظات النادرة",
        "   ⚠️ حظر مطلق: لا تضع أبداً أسماء الشخصيات أو مستويات الطاقة داخل أي وسوم.",
        "   ⚠️ أي وسم خارج القائمة أعلاه (مثل [cultivation] أو [character] أو [name]) = خطأ فادح.",
        "",
        "4. صيغة الإخراج الحصرية (JSON Array فقط):",
        "   - ردّك يجب أن يكون مصفوفة JSON صالحة فقط، بلا أي نصوص قبلها أو بعدها.",
        "   - البنية المطلوبة:",
        "[",
        "  {",
        "    \"chapNum\": 101,",
        "    \"title\": \"الفصل 101: العنوان الفصيح\",",
        "    \"content\": \"نص الفصل المصقول كاملاً...\",",
        "    \"score\": 96,",
        ...
        "",
        "══════════════════════════════════════════════════",
        "📖 [قاموس المصطلحات والشخصيات لهذه الدفعة]:",
        "══════════════════════════════════════════════════",
    ]
    ...
    for g in matched_glossary:
        ...
        gender_raw = g.get('gender', '')
        gender = f" [{'ذكر' if gender_raw in ('male', 'ذكر') else 'أنثى'}]" if gender_raw else ""
        prompt_lines.append(f"• {g['original']} → {g['arabic']}{cat}{gender}")

    prompt_lines += [
        "",
        "══════════════════════════════════════════════════",
        "📜 [نصوص الفصول المطلوب صقلها]:",
        "══════════════════════════════════════════════════",
    ]
    ...
    logger.info(f"📝 تم توليد ملف البرومبت الروائي الذهبي مع القاموس المفلتر: {prompt_file}")

    # توليد ملف الدفعة المجمعة مع الفصول
    chapter_blocks = []
    for pf in pending_files:
        meta, txt = parse_chapter_file(pf)
        c_num = meta.get("chapter", re.search(r'\d+', pf.name).group(0))
        c_title = meta.get("title", f"الفصل {c_num}")
        chapter_blocks.append(f"=== CHAPTER_START: {c_num} ===\nTITLE: {c_title}\nCONTENT:\n{txt}\n=== CHAPTER_END ===")

    full_bundle = prompt_text + "\n\n" + "\n\n".join(chapter_blocks)
    bundle_file = STAGING_DIR / "CLAUDE_FULL_BATCH_BUNDLE.txt"
    bundle_file.write_text(full_bundle, encoding="utf-8")
